...
import com.ryanjames.swabergersmobilepos.databinding.ViewRowItemHeaderBinding
import com.ryanjames.swabergersmobilepos.databinding.ViewRowItemSelectBinding
...
private const val ID_PRODUCT_GROUP_HEADER = 4

class MenuItemDetailAdapter2(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        val binding = ViewRowItemSelectBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return when (viewType) {
            ID_MEAL_OPTION -> RowSelectMealViewHolder(
                binding,
                onClickRowListener
            )
            ID_PRODUCT_GROUP -> RowSelectProductGroupViewHolder(
                binding,
                onClickRowListener
            )
            ID_PRODUCT_GROUP_HEADER -> {
                val headerBinding = ViewRowItemHeaderBinding.inflate(LayoutInflater.from(parent.context), parent, false)
    ...
    class RowSelectMealViewHolder(
        val binding: ViewRowItemSelectBinding,
        private val onClickRowListener: OnClickRowListener
    ...
    class RowSelectProductGroupModifierViewHolder(
        ...
        fun bind(product: Product, modifierGroup: ModifierGroup, modifierList: List<ModifierInfo>, hideLine: Boolean) {
            binding.tvHeader.text = modifierGroup.modifierGroupName
    ...
    class RowProductGroupHeaderViewHolder(
        private val binding: ViewRowItemHeaderBinding
    ) : RecyclerView.ViewHolder(binding.root) {
